...
export function SynonymGroupBuilder (sparqlEndpoint: SparqlEndpoint, taxonName: string, ignoreRank = false): AsyncIterator<JustifiedSynonym> {
  const justifiedArray: JustifiedSynonym[] = []
  /** Maps from taxonConceptUris to their synonyms */
  const justifiedSynonyms: Map<string, number> = new Map()

  const buffer: (JustifiedSynonym|true)[] = []
  const resolverBuffer: ((_: JustifiedSynonym|true) => void)[] = []
  function resolver (value: JustifiedSynonym|true) {
    if (resolverBuffer.length) {
      resolverBuffer.shift()!(value)
    } else {
      buffer.push(value)
    }
  }
  ...
  return {
    next () {
      return new Promise(resolve => {
        function _resolver (value: JustifiedSynonym | true) {
          if (value === true) {
            resolve({ done: true, value: new SynonymGroup(justifiedArray) })
          } else {
            resolve({ value })
          }
        }
        ...
        if (buffer.length) {
          _resolver(buffer.shift()!)
        } else {
          resolverBuffer.push(_resolver)
        }
      })
    }
  }
